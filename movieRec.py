...
def recommend(title):
    ...
    for index, row in df.iterrows():
        ...
        key_words_dict_scores = (
            r.get_word_degrees()
        )  # to get dictionary with key words and their similarity scores
        keywords.append(
            list(key_words_dict_scores.keys())
        )  # to assign it to new column
    ...
    top_10_indices = list(
        score_series.iloc[1:11].index
    )  # to get the indices of top 6 most similar movies

    for (
        i
    ) in (
        top_10_indices
    ):  # to append the titles of top 10 similar movies to the recommended_movies list
